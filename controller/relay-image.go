...
func relayImageEditHelper(c *gin.Context, relayMode int) *OpenAIErrorWithStatusCode {

	//fileData, err := ioutil.ReadFile(filePath)
	//if err != nil {
	//	return errorWrapper(err, "file_upload_error", http.StatusBadRequest)
	//}
	...
	var imageRequest ImageEditsRequest
	...
	//imageRequest.Image = base64Encoded
	...
	//}
	var requestBodyWriter bytes.Buffer
	...
	fields := []struct {
		...
		{"n", fmt.Sprintf("%d", imageRequest.N)},
	...
	for _, field := range fields {
		err := multipartWriter.WriteField(field.Field, field.Value)
		if err != nil {
			return errorWrapper(err, "multipartWriter_write_filed_error", http.StatusInternalServerError)
		}
		fieldValues[field.Field] = field.Value
	}
	filePath := "C:/Users/shixd/Pictures/p.png"
	fileName := "p.png"
	...
	defer file.Close()
	fileFieldWriter, err := multipartWriter.CreateFormFile("image", fileName)
	if err != nil {
		return errorWrapper(err, "multipartWriter_write_filed_error", http.StatusInternalServerError)
	}
	_, err = io.Copy(fileFieldWriter, file)
	if err != nil {
		return errorWrapper(err, "multipartWriter_write_filed_error", http.StatusInternalServerError)
	}
	...
	req, err := http.NewRequest(c.Request.Method, fullRequestURL, &requestBodyWriter)
	if err != nil {
